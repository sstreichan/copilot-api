...
describe("Anthropic to OpenAI translation logic", () => {
  ...
  test("should flatten single text block assistant content to a string for glm models", () => {
    const anthropicPayload: AnthropicMessagesPayload = {
      model: "glm-5.3",
      messages: [
        { role: "user", content: "Summarize the fix." },
        {
          role: "assistant",
  ...
  test("should flatten single text block assistant content to a string for glm models with tool calls", () => {
    const anthropicPayload: AnthropicMessagesPayload = {
      model: "glm-5.3",
      messages: [
  ...
  test("should keep assistant content as an array for glm models when it has multiple text parts", () => {
    const anthropicPayload: AnthropicMessagesPayload = {
      model: "glm-5.3",
      messages: [
  ...
  test("should keep single text block assistant content as an array for non-glm models", () => {
    const anthropicPayload: AnthropicMessagesPayload = {
      model: "gpt-4o",
      messages: [
        { role: "user", content: "Summarize the fix." },
