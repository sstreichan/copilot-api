...
let createChatCompletionsSpy: ReturnType<
  typeof spyOn<typeof createChatCompletionsModule, "createChatCompletions">
>

const fakeCreateChatCompletions = (
...
beforeEach(() => {
  ...
  createChatCompletionsSpy = spyOn(
    createChatCompletionsModule,
    "createChatCompletions",
  ).mockImplementation(fakeCreateChatCompletions)
})

afterEach(() => {
  createChatCompletionsSpy.mockRestore()
})
...
test("messages Chat Completions flow adds Copilot cache control to system and latest two non-system messages", async () => {
  ...
  expect(createChatCompletionsSpy).toHaveBeenCalledTimes(1)
