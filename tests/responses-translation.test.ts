...
describe("translateAnthropicMessagesToResponsesPayload", () => {
  ...
  it("does not add assistant phase for models without intermediary update prompts", () => {
    const payload = {
      model: "unknown-model",
      max_tokens: 1024,
      messages: [
  ...
  it("adds assistant phase for models with intermediary update prompts", () => {
    const payload = {
      model: "gpt-5.4",
      max_tokens: 1024,
      messages: [
    ...
    expect(input[0]?.phase).toBe("final_answer")
    expect(input[1]?.phase).toBe("commentary")
  })
