...
const zh: Locale = {
  ...
  advancedConfig: {
    title: '高级配置',
    subtitle: '通过正在运行的服务接口管理模型映射，保存后新的 `POST /v1/messages` 请求会立即使用最新规则。',
    back: '返回看板',
    modelMappingsTitle: '模型映射',
    modelMappingsDesc: '把请求里的模型名映射成另一个目标模型。目标值既可以是普通模型 ID，也可以是 provider/model。',
    scopeLabel: '作用范围',
    scopeNote: '仅作用于 `POST /v1/messages`。匹配为精确匹配，应用不会自动帮你裁剪空格或规范化大小写。',
    ...
    remove: '删除',
    emptyTitle: '还没有映射规则',
    emptyDescription: '新增一条规则后，请求里的模型名会在 Messages API 解析实际上游模型前先被改写。',
    saveHelp: '保存时会通过服务接口只更新 config.json 里的 modelMappings 字段，不会覆盖其他配置。',
    saved: '已通过服务接口保存。后续新的 `POST /v1/messages` 请求会立即使用最新映射。',
    loadFailed: '加载模型映射失败',
    saveFailed: '保存模型映射失败',
