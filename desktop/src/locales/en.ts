...
const en: Locale = {
  ...
  advancedConfig: {
    ...
    subtitle: 'Manage model mappings through the running service API so new POST /v1/messages requests pick up changes immediately.',
    back: 'Back to dashboard',
    modelMappingsTitle: 'Model mappings',
    modelMappingsDesc: 'Map the requested model name to another target model. Targets may be regular model IDs or provider/model aliases.',
    scopeLabel: 'Scope',
    scopeNote: 'Only applies to POST /v1/messages. Matching is exact and the app will not trim or normalize the model name for you.',
    ...
    emptyDescription: 'Add a rule to remap an incoming model name before the Messages API resolves the actual upstream model.',
    saveHelp: 'Only the modelMappings field is updated through the service API. Other config values remain untouched.',
    saved: 'Saved through the service API. New POST /v1/messages requests will use the updated mappings immediately.',
    loadFailed: 'Failed to load model mappings',
    saveFailed: 'Failed to save model mappings',
    serverRequired: 'Start the local service before loading or saving advanced config through the API.',
