...
| Usage / smart agent input | `get-copilot-usage.ts` | 配额查询与 forceAgent 决策 |
...
- `getSmartAgentDecision()` 在取数失败时返回 `forceAgent: true` 保护配额；不要把失败默默当成“预算正常”
...
- `getCopilotUsage()` 同时是 usage viewer 和 smart-agent 的数据源；改返回接口时要一起检查 `quota_snapshots.premium_interactions`
...
- 把 smart-agent 预算判断塞回 handler 或 route 层，绕开 `get-copilot-usage.ts`
